...
class StockNewsSentiment(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, null=True)
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=['user', 'stock_name', 'last_refresh_data'],
                name='unique_user_stock_news_sentiment'
            )
        ]
...
class StockNews(models.Model):
    ...
    def __str__(self):
        return f"{self.stock_name} - {self.headline[:40]}"


class UserSentimentStatus(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE)
