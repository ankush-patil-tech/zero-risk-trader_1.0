...
def predict_df(request):
    from .aiml.prediction_models import train_linear_model, predict_next_day, train_decision_tree_model, \
        train_random_forest_model, train_svm_model, predict_next_day_svm, calculate_model_scores, \
        calculate_success_rate, calculate_directional_success_rate, calculate_avg_error
    # , train_lstm_model, predict_next_day_lstm, save_lstm_to_db, load_lstm_from_db,
...
#     # 1️⃣ Fetch & store raw news
#     store_news_for_user(request.user)
#
#     # 2️⃣ Analyze stored news
#     process_news_for_user(request.user)
#
#     return JsonResponse({"status": "News fetched & sentiment analyzed"})

# auth_app/views.py
from django.http import JsonResponse
from django.contrib.auth.decorators import login_required
from auth_app.utils.sentiment_runner import run_sentiment_background
...
@login_required
def ai_ml_summary(request):
    raw_stock = request.GET.get("stock")
    user = request.user
    ...
    try:
        ...
        news_signal = news.meta_data.get("final_signal")
